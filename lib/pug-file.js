var attrs = require('pug-attrs')
  ...
  , PugFile = function (filename, source) {
      ...
      try {
        this._tokens = lexer(this._source, this._filename).map(function (token, index) {
          token._index = index
          return token
        })
      } catch (e) {
...
PugFile.prototype =
  ...
  , addErrorForConcatenation: function (token, errors, message) {
    ...
    var regex = new RegExp(utils.concatenationRegex)
      ...
      if (match !== null) {
        columnNumber = line.indexOf(token.val) + 1 + match.index + match[0].indexOf(match[3] || match[8])
