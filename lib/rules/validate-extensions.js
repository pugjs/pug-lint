...
module.exports.prototype = {
  ...
  lint(file, errors) {
    const tokens = file.getTokensByFilter(token => token.type === 'include' || token.type === 'extends' || token.type === 'path');
    ...
    for (const [i, token] of tokens.entries()) {
      ...
      } else /* istanbul ignore else */ if (token.type === 'extends' && path.extname(next.val) !== '.pug') {
