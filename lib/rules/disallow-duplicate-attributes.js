...
module.exports.prototype = {
	...
	lint(file, errors) {
		file.iterateTokensByType('start-attributes', (token) => {
			...
			const attributesEnd = file.getNextTokenByType(
				token,
				'end-attributes'
			);
			const attributesEndIndex = attributesEnd._index;
			const attributesLineTokens = file.getTokensByFilter(
				(token) => token.line === attributesStart.line
			);
			const boundaryStart =
				file.getPreviousTokenByType(
					token,
