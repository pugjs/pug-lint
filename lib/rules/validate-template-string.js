...
module.exports.prototype = {
  ...
  lint(file, errors) {
    ...
    file.iterateTokensByFilter(token => (
      ...
      if (_this._subrules.string || _this._subrules.variable) {
        file.addErrorWithAcorn(token, ast => {
          const out = [];
          ...
          acornWalk.ancestor(ast, {
            TemplateLiteral(node, ancestors) {
              ...
              const canBeString = _this._subrules.string
                && !node.expressions.length;
              const canBeVariable = _this._subrules.variable
      ...
      if (_this._subrules.concatenation) {
        file.addErrorWithAcorn(token, (ast, tokens) => {
          const out = [];
