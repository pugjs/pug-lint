    ...
    function addErrorForDisallowedAttribute(values, lineNumber, tag) {
      ...
      for (const value of values) {
        if (typeof value === 'object') {
          file.iterateTokensByFilter(token => token.type === 'tag' && utils.ownProperty(value, token.val) !== null, token => {
            addErrorForDisallowedAttribute(utils.ownProperty(value, token.val), token.line, token.val);
          });
        } else {
          file.iterateTokensByFilter(token => token.type === 'attribute' && (token.line === lineNumber || lineNumber === undefined), token => {
            const {name, line, col} = token;
            let errorMessage;

            if (name.toLowerCase() === value.toLowerCase()) {
              errorMessage = tag ? 'Tag "' + tag + '" must not have attribute "' + name + '"' : 'Attribute "' + name + '" must not be used';

              errors.add(errorMessage, line, col);
            }
          });
        }
      }
