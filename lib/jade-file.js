...
JadeFile.prototype =
  ...
  , addErrorForIncorrectTokenTypeOrder: function (typeA, typeB, boundaryTypes, errors, message) {
      ...
      _this.iterateTokensByType(typeB, function (token) {
        var current = token
          ...
          , start = _this.getPreviousTokenByType(token, boundaryTypes) || token
  ...
  , getNextTokenByType: function (current, type, direction) {
      ...
      while (next === undefined && current !== null && index >= 0) {
        index = direction === 'next' ? index + 1 : index - 1
        current = this.getToken(index)

        if (current !== undefined && current !== null && type.indexOf(current.type) !== -1) {
          next = current
        }
      }
