...
exports.loadFromFile = function (configPath) {
  ...
  if (fs.existsSync(configPath)) {
    ...
    content = ext === '.js' ? require(configPath) : JSON.parse(stripJSONComments(fs.readFileSync(configPath, 'utf8')));
...
exports.loadFromHomeDirectory = function () {
  const directoryArray = [process.env.USERPROFILE, process.env.HOMEPATH, process.env.HOME];
